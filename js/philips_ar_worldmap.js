...
jQuery(function () {
	...
	// Overrides the region mouseover event (set 'selected' property for all regions in mapData[i].code array)						
		handleRegionMouseOver = function (e, code) {
			for (var i = 0; i < mapData.length; i++) {
				if ($.inArray(code, mapData[i].code) > -1) {
					//debugger;
					map.setSelectedRegions(getMapCodes(mapData[i].code));
					break;
				}
			}
		},
	...
	//returns an array of country codes that exist in the definition of the maps object
		getMapCodes = function (dataArray) {
			...
			if (availableCountryCodes.length == 0) availableCountryCodes = getAvailableCountryCodes()

			for (var i = 0; i < dataArray.length; i++) {
				var countryCodeToFind = dataArray[i];
				for (var j = 0; j < availableCountryCodes.length; j++) {
					var currentCoutryCode = availableCountryCodes[j];
	...
	// Overrides the default country popup (renders html based on data from the countriesData object)				
		showCountryDetails = function (e, el, code) {
			...
			var regionData = null;
			$.each(mapData, function (index, obj) {
			...
			if (regionData != null) {
				...
				switch (mapVariation) {
					case 'lives_improved':
						...
						regionHtml = regionHtml.replace(/\[total_value]/g, format(getCategoriesTotal(regionData.categories)));
		...
		generateColors = function (mapData) {
			...
			for (region in map.regions) {
				...
				if (regionData) {
					...
					colors[region] = mapForeGroundColor;
				}
			}
			return colors;
		},
			...
			getMapData(mode, function (mapData) {
				...
				map = new jvm.WorldMap({
					...
					regionStyle: {
						...
						selected: {
							fill: regionHoverColor,
							"fill-opacity": regionHoverOpacity
						}
					},
					onRegionOver: function (e, code) {
						handleRegionMouseOver(e, code);
					},
					onRegionSelected: function (e, code, isSelected, selectedRegions) {
						handleRegionSelected(e, code, isSelected, selectedRegions);
					},
					onRegionOut: function (e, code) {
						map.clearSelectedRegions();
					},
					onRegionLabelShow: function (e, el, code) {
						showCountryDetails(e, el, code);
					},
					series: {
						regions: [{
							hoverColor: true
				...
				map.series.regions[0].setValues(generateColors(mapData));
